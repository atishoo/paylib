...
class Alipaylib{
    ...
    private $partner;                //appKey
    private $private_key;             //secret
    private $public_key;
    private $service;
    private $sign_type;
    ...
    public function rsaVerify($data, $sign)  {
        //以下为了初始化私钥，保证在您填写私钥时不管是带格式还是不带格式都可以通过验证。
        $alipay_public_key=str_replace("-----BEGIN PUBLIC KEY-----","",$this->public_key);
    ...
    private function completeParams($data)
    {
        $data['service'] = '"'.$this->service.'"';
        $data['partner'] = '"'.$this->partner.'"';
        $data['_input_charset'] = '"'.$this->input_charset.'"';
        $data['payment_type'] = '"'.(1).'"';
        $data['seller_id'] = '"aotongxinxikeji@gmail.com"'; // 卖家支付宝账号（邮箱或手机号码格式）或其对应的支付宝唯一用户号（以2088开头的纯16位数字）
        $data['service'] = '"'.$this->service.'"';
        return $data;
    }
    ...
    public function notifyToUrlParams($para) {
        $arg  = "";
        while (list ($key, $val) = each ($para)) {
            $arg.=$key."=".$val."&";
        }
    ...
    function getSignVeryfy($para_temp, $sign) {
        ...
        //把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
        $prestr = $this->notifyToUrlParams($para_sort);
